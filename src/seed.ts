...
async function seed() {
  ...
  await AppDataSource.transaction(async (manager) => {
    const globalVisaRepo = manager.getRepository(GlobalVisaConfiguration);
    const globalVisaFileRepo = manager.getRepository(
    ...
    for (const file of filesToSeed) {
      ...
      if (!visa) {
        console.log(
          `⚠️  Skipping file ${file.file_name} - visa configuration not found: ${file.visa_name}`,
        );
        continue;
      }
      ...
      if (!existing) {
        ...
        console.log(
          `ℹ️  File already exists: ${file.file_name} for ${file.visa_name}`,
        );
      }
    }
  });
